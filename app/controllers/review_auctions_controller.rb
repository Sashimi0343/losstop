class ReviewAuctionsController < ApplicationController
  ...
  def review_params
    if current_user.nil?
      params.require(:review_auction).permit(:comment).merge(name: current_company.name,
                                                             auction_id: params[:auction_id], company_id: current_company.id)
    else
      params.require(:review_auction).permit(:comment).merge(name: current_user.name, auction_id: params[:auction_id],
